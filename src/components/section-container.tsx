...
const CONTAINER_VARIANTS = {
  base: cn("relative", "z-10", "w-full"),

  centered: "mx-auto",

  gutters: cn("px-6", "sm:px-8", "lg:px-10", "xl:px-12", "2xl:px-18"),
...
function buildContainerClassName(config: SectionConfig): string {
  const fullWidth = config.width === "full" || config.fluid;

  return cn(
    CONTAINER_VARIANTS.base,

    !fullWidth && CONTAINER_VARIANTS.centered,

    !config.disableGutters
      ? CONTAINER_VARIANTS.gutters
      : CONTAINER_VARIANTS.noGutters,

    !fullWidth && WIDTH_VARIANTS[config.width],
